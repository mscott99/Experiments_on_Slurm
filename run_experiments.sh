...
CPU_NUM="1"
GPU_NUM="0"
ROWS_PER_WORKER=40 # 40 for sparse, 20 for gen MNIST.
...
read -r NUM_WORKERS CPU_NUM GPU_NUM MEMORY TIME ROWS_PER_WORKER <<< $(python "$SWEEP_FILE" --setup -f "$EXPERIMENT_MODULE" --rows-per-worker "$ROWS_PER_WORKER" -o "$OUT_DIR") || {
    rm -f "$BASE_OUT_DIR"/../running.lock
    exit 1
}
